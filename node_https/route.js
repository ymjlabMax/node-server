...
function cyrb128(str, date) {
    ...
    let h1 = (1779033703 - date), h2 = 3144134277,
...
router.get('/shopHistory', function(req, res) {
	var authKey = req.query.authKey;	//'3947429038';
	var nickName = req.query.nickName; //'alfie96';
	var day = req.query.baseday;
	...
	console.log(nickName, baseday, authKey);
	...
	pool.getConnection(function(err,connection){
		...
		connection.query("insert into shop (authKey, userId, baseday, result, regDate) values (?,?,?,?, now())", [authKey, nickName, baseday, rlt], function(err, rows) {
			if(err) {
				connection.release();
